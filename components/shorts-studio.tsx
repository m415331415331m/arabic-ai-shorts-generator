...
export default function ShortsStudio() {
  ...
  const analyze = async () => {
    ...
    if (!value) { setMessage('ألصق رابط يوتيوب أولاً.'); return }
    setStatus('analyzing'); setMessage('جارٍ تحليل الرابط وتجهيز المعاينة...')
    try {
      const res = await fetch('/api/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: value }) })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'تعذر تحليل الرابط')
      if (data.caption) { setCaption(data.caption); setClip({ ...data, caption: data.caption }) }
      setMessage(data.demo ? 'تمت المعاينة والتحليل التجريبي. أضف ملف المصدر لتفعيل الرندر الحقيقي.' : 'تم التحليل بنجاح. أضف ملف المصدر لتصدير MP4 حقيقي.')
    } catch (error) {
